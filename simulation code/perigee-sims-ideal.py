...
# nodes update one at a time with the distance function updating in-between
...
def update_just_dist(m):
    return g.dijkstra_all().copy()
...
def run_simulation(num_nodes,num_miners,d_out,r,d_in,k, dist = []):
    ...
    # for each round, each miner drops the worst peer and makes a new random peer **one at a time**
...
dir = 'perigee-ideal-simulations/'

# run  simulations
file_name = 'perigee_ideal_n_'+str(n)+'_m_'+str(m)+'_d_'+str(d_out)+'_din_'+lim+'_r_'+str(r)+'_k_'+str(k)+'.json'
for i in range(sims):
    s = datetime.now()
